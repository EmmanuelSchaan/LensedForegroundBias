...
#      fig.savefig("./figures/velocities/variance_of_vrms2_spherical.pdf", bbox_inches='tight')
   ...
   def fPqr(self, k, z):
      ...
      def integrand(par):
         x = par[0]  # such that |p| = x |k|
         mu = par[1] # such that mu = cos(theta_{k, p})
         #
         result = self.fPlin_z(k*x, z)
         result *= self.fPlin_z(k*np.sqrt(1.+x**2-2.*x*mu), z)
      ...
      xMin = 1.e-3 / k
      xMax = 1.e1 / k
      integ = vegas.Integrator([[xMin, xMax], [-1., 1.]])
      result = integ(integrand, nitn=10, neval=500)
      print result.sdev / result.mean
#      print result.summary()
      ...
      result *= (self.Hubble(1/(1.+z)) / 3.e5 * (self.OmM*(1.+z)**3)**(5./9.) / (1.+z))**2 # (aHf/c)^2 [(h/Mpc)^2]

...
#!!!!!!!!!!!!!!!!!!!!!!!! value of self.L0? Units?
   ...
   def L500(self, nu, m500, a):
      result = self.L0 * (m500/1.e14*self.U.h)**self.eCIB
      result *= self.psi(a)
...
class ProfGasBattaglia16(Profile):
   ...
   def __init__(self, U, trunc=2.):
      ...
      self.rho0 = lambda m,z: 4.e3 * ((m/self.U.h)/1.e14)**0.29 * (1.+z)**(-0.66)
      self.alpha = lambda m,z: 0.88 * ((m/self.U.h)/1.e14)**(-0.03) * (1.+z)**0.19
      self.beta = lambda m,z: 3.83 * ((m/self.U.h)/1.e14)**0.04 * (1.+z)**(-0.025)
   ...
   def rho3d(self, r, m, z):
      ...
      result *= self.U.rhocrit_z(z)
      # rescale with the baryon fraction
      result *= self.U.OmB/self.U.OmM
      return result
   ...
   def rho3dFourier(self, k, m, z):
      ...
      result *= r200c**2
      result *= self.U.rhocrit_z(z) * self.U.OmB/self.U.OmM
      return result
   ...
   def tau3d(self, r, m, z):
      ...
      sigma_T *= (self.U.h/mpc)**2  # in (Mpc/h)^2
   ...
   def tauBeam(self, R, m, z, fwhm):
      ...
      sigmaBeam *= self.U.ComovDist(1./(1.+z), 1.) # in comoving Mpc/h
